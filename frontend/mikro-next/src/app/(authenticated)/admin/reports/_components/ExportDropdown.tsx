...
import { exportReportAsCsv, exportChartsAsZip, exportChartsAsDocx } from "@/lib/reportExport";
import type {
...
export function ExportDropdown({
  ...
  function handleCsv() {
    setOpen(false);
    exportReportAsCsv(timekeepingData, editingData, elementCategories, dateRange);
  }
    ...
    <div className="relative" ref={menuRef}>
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-44 rounded-lg border border-border bg-card shadow-lg z-50 overflow-hidden">
          ...
          <button
            onClick={() => handleExport(exportChartsAsZip)}
            className="w-full text-left px-4 py-2.5 text-sm hover:bg-muted transition-colors flex items-center gap-2"
          >
