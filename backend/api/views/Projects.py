...
class ProjectAPI(MethodView):
    ...
    def create_project(self):
        ...
        required_args = ["url", "rate_type", "rate", "max_editors", "visibility"]
        ...
        if not m:
            return {"message": "Cannot get URL from project info", "status": 400}
    ...
    def update_project(self):
        ...
        if not project_status:
            ...
            project_status=True
        target_project=Project.query.filter_by(org_id=g.user.org_id, id= project_id).first()
        ...
        #Put logic here to process remaining payouts or whatever else before deletion
        response['status']=200
        return response
    ...
    def calculate_budget(self):
        ...
        # Calculate payment
        total_tasks = json_data["totalTasks"] if matcher == "tm3" else len(json_data["tasks"]["features"])
        if rate_type == True:
            rate = float(rate)
            dollars = int(rate)
            ...
            return_text = f"${rate:.2f} x {total_tasks} Tasks = Projected Budget: ${projected_budget:.2f}"
            ...
            return_text = f"${rate:.2f} / {total_tasks} tasks =  ${calculation:.2f} per task."
            adjust_budget_text = f"  - Recommended adjusted budget = ${adjusted_budget:.2f}"
            if calculation < 0.01:
                return_text = f"${rate:.2f} / {total_tasks} tasks =   less than $0.01 per task."
    ...
    def fetch_org_projects(self):
        ...
        active_projects = Project.query.filter_by(org_id=g.user.org_id,status=True).all()
        inactive_projects = Project.query.filter_by(org_id=g.user.org_id,status=False).all()
...
## USER ROUTES
    ...
    def fetch_user_dash_stats(self):
        ...
        if not g:
            return {'message': 'User not found', 'status': 304}
        all_user_assignments_count=len(ProjectUser.query.filter_by(user_id=g.user.id).all())
        all_user_assignment_ids=[relation.project_id for relation in ProjectUser.query.filter_by(user_id=g.user.id).all() ]
        # Retrieve all projects and tasks for the organization
        all_projects = Project.query.filter_by(org_id=g.user.org_id).all()
        all_tasks= Task.query.filter_by(org_id=g.user.org_id).all()
        ...
        all_user_payments = Payments.query.filter_by(org_id=g.user.org_id,user_id=g.user.id).all()
        # Compute various statistics
        active_projects_count = sum(project.status for project in all_projects)
    ...
    def fetch_user_projects(self):
        ...
        org_active_projects = []
        org_inactive_projects = []
        all_user_project_ids=[relation.project_id for relation in ProjectUser.query.filter_by(user_id=g.user.id).all()]
        user_joined_projects = [project for project in Project.query.filter_by(org_id=g.user.org_id,status=True).all() if project.id in all_user_project_ids]
        user_available_projects = [project for project in Project.query.filter_by(org_id=g.user.org_id,status=True).all() if project.id not in all_user_project_ids and project.total_editors<project.max_editors]
        # Add each project to the list
    ...
    def user_join_project(self):
        ...
        return {'message': 'User %s has joined project %s'%(g.user.id,project_id), 'status': 200}
    ...
    def user_leave_project(self):
            ...
            if not project_id:
                return {"message": "project_id required", "status": 400}
            target_relation=ProjectUser.query.filter_by(project_id=project_id, user_id = g.user.id).first()
            ...
            return {'message': 'User %s has left project %s'%(g.user.id,project_id), 'status': 200}
